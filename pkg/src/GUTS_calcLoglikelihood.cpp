...
void GUTS::calcLoglikelihood() {
  ...
  /*
   * We need survival probabilities (mS) first. calcSurvProb will also
   *  call calcSample().
   */
  calcSurvivalProbabilities();

  /*
   * Reset error on this method, and check for new errors.
   * If new errors, we will return
   */
  ...
  for (unsigned int j = 0; j < 2; ++j) {
    if (mErrors.at(i[j])) {
      ...
      mErrorMessages.at(GER_LL) = "calcLoglikelihood failed: "
