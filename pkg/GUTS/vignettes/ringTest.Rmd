...
The ring test data set A-SD is read from file "Data for GUTS software ring test v05.xlsx".

The respective data table is in wide format. Here is the display as an R data.frame:
```{r A-SD-read-all-comments}
library(xlsx)
read.xlsx(
...
mcmc_result_SD <- MCMC(p = logposterior, 
  init = pars_start_SD, adapt = 2500, acc.rate = 0.4, n = 20000, 
  guts_objects = GUTS_A_SD, 
  isOutOfBoundsFun = is_out_of_bounds_fun_SD
)

#exclude burnin and thin by 3
mcmc_result_SD$samples <- mcmc_result_SD$samples[seq(5001, 20000, by = 3),]
mcmc_result_SD$log.p <- mcmc_result_SD$log.p[seq(5001, 20000, by = 3)]
```

### Chain and distribution plots. 
The top four graphs show mixing and distribution of the parameters. The last row "LL" show mixing and distribution of the logposterior.
...
The ring test data set A-IT is read from file "Data for GUTS software ring test v05.xlsx".
...
* hard boundaries: 
    ...
    + $p<0$ confines rates and thresholds to positive values
* knowledge on numerical limitations: 
    + $exp(8/p[4]) * p[3] > 1e200$ avoids a numerical problems when approximating the log-logistic function for unrealistically high median values $p[3]$ and extremely low shape values $p[4]$ that result in a threshold distribution confined at a zero threshold.
...
mcmc_result_IT <- MCMC(p = logposterior, 
  init = pars_start_IT, adapt = 2500, acc.rate = 0.4, n = 20000, 
  guts_objects = GUTS_A_IT, isOutOfBoundsFun = is_out_of_bounds_fun_IT
)

#exclude burnin and thin by 3
mcmc_result_IT$samples <- mcmc_result_IT$samples[seq(5001, 20000, by = 3), ]
mcmc_result_IT$log.p <- mcmc_result_IT$log.p[seq(5001, 20000, by = 3)]
```
...
According to the protocol for the ring test, for prediction the background mortality "hb" was fixed at 0, in order to isolate the treatment effects in model forecasts. Further, to speed up calculation, we thin the posterior sample and use only each 5th parameter set.
