---
title: "Analysing Diazinon impact on the freshwater crustacean Gammarus pulex using GUTS-SIC-proper"
author: "Alexander Singer"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Analysing Diazinon impact on the freshwater crustacean Gammarus pulex using GUTS-SIC-proper}
...
* the GUTS-model type `model = "Proper"`
* the tolerance threshold distribution `dist = "loglogistic"`
...
```{r load optimization routines, echo = TRUE, results = 'hide'}
...
* hard boundaries: 
    + `is.infinite(p)` constraints parameters to finite values
    + `p<0` confines rates and thresholds to positive values
* knowledge on numerical limitations:
    + `p[3] > 30` confines the killing rate to realistic values. The constraint avoids an unrealistic local optimum of the optimization problem, i.e. a high killing rate simulates the effect of a low tolerance threshold, and makes the threshold estimation obsolete. 
    + `p[5] <= 1` is assumed, to exclude that the mode of the log-logistic threshold distribution is 0, due to the shape parameter. Note that the mode can approach 0 if the estimated median `p[4]` approaches 0. Therefore, this constraint stabilises estimation of low threshold values.
    + `exp(8/p[5]) * p[4] > 1e200` avoids numerical problems when approximating the log-logistic function for unrealistically high median values `p[4]` and extremely low shape values `p[5]` that result in a threshold distribution confined at a zero threshold
* toxicological or ecological information: not used here

```{r define-out-of-bounds-fun-Proper}
...
```{r guess-initial-values-Proper, echo = TRUE, eval = do.calc}
...
```{r run-MCMC-Proper, echo = TRUE, results = 'hide', eval = do.calc}
...
```{r save-MCMC-results-Proper, echo = FALSE, results = 'hide', eval = do.calc & do.save}
save(mcmc_result_Proper, file = file.path("..", "inst", "extdata", "vignetteGUTS-Proper-MCMCresults.Rdata"))
```

```{r load-MCMC-results-Proper, echo = FALSE, results = 'hide', eval = !do.calc}
...
* the initial number of individuals `y[1]` in a vector `y` of the same length as `yt`. Elements other than `y[1]` can be set arbitrarily. The values are ignored in forecasts.
...
memory.limit(16000) # for convenient treatment of large projection objects
...
```{r extract-forecast-results-Proper, echo = TRUE, results = 'hide', eval = do.calc}
# extract the damage matrix
damage <- sapply(forec, function(x) x[[2]])
forec <- sapply(forec, function(x) x[[1]])

# analyse damage
damage.qu <- apply(damage, 1, function(x) quantile(x, probs = c(0.025, 0.5, 0.975)))
damage.ti <- guts_report_damage(gobj = guts_obj_forecast)$time

# coerce the forec-matrix to a vector for plotting
dat <- cbind(rep(seq_len(dim(forec)[1]) - 1, times = dim(forec)[2]), c(forec, recursive = TRUE))

deaths <- 100 * (forec[1:dim(forec)[1] - 1,] - forec[2:dim(forec)[1],])
dat.deaths <- cbind(rep(seq_len(dim(deaths)[1]), times = dim(deaths)[2]), c(deaths, recursive = TRUE))
```

```{r save-forecast-Proper, echo = FALSE, results = 'hide', eval = do.calc & do.save}
save(dat, damage.qu, damage.ti, dat.deaths, file = file.path("..", "inst", "extdata", "vignetteGUTS-Proper-forecast.Rdata"))
```

```{r load-forecast-Proper, echo = FALSE, results = 'hide', eval = !do.calc}
...
## Analyse projections
The plots show (i) measured concentrations, (ii) damage (median and 95%-CI), (iii) survival probability and (iv) the number of death individuals over time.

```{r plot-forecast}
par(mfrow = c(4,1), mar = c(5,4,0.5, 0.5))
plot(guts_obj_forecast$Ct, guts_obj_forecast$C, xlim = range(guts_obj_forecast$yt), xlab = "day", ylab = "dose", pch = 20, cex = 1.5)
plot(damage.ti, damage.qu[2,], type = "l", ylim = range(damage.qu), xlab = "time", ylab = "damage D")
lines(damage.ti, damage.qu[1,], lty = 2)
lines(damage.ti, damage.qu[3,], lty = 2)
plot(as.factor(dat[,1]), dat[,2],
      xlab = "day", ylab = "probability of survival"
)
plot(as.factor(dat.deaths[,1]), dat.deaths[,2],
      xlab = "day", ylab = "deaths"
)
```

The time series demonstrates an accumulative effect: The second application exerts higher mortality than the first application due to accumulating damage. The third application causes only few further deaths, because (i) only few individuals have survived until time step 20 and (ii) their individual tolerance thresholds are high.
